...
class ChromeDebuggerConnection constructor(
    ...
    private val eventNameToClassMapping: ConcurrentHashMap<String, Class<out Event>> = ConcurrentHashMap()
    private val nextRequestId = AtomicLong(0)

    /**
     * Registers event name to class mappings.
     */
    override fun registerEventMappings(mapOf: Map<String, Class<out Event>>) {
     ...
     */
    override fun <T> captureEvents(name: String, outClazz: Class<T>): Flowable<Timed<T>> where T : Event {
     ...
     */
    override fun captureAllEvents(): Flowable<Timed<Event>> {
        return frames.eventFrames()
            .flatMapSingle { frame ->
                mapper.deserializeEvent(frame.value(), eventNameToClassMapping[frame.value().method] ?: Event::class.java).map {
                    Timed(it, frame.time(), frame.unit())
                }
            }
            .subscribeOn(Schedulers.io())
