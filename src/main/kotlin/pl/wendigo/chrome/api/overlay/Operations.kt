...
class OverlayOperations internal constructor(private val connection: pl.wendigo.chrome.protocol.ChromeDebuggerConnection) {
    ...
    fun disable(): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.disable", null, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
    ...
    fun enable(): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.enable", null, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
    ...
    fun getHighlightObjectForTest(input: GetHighlightObjectForTestRequest): io.reactivex.Single<GetHighlightObjectForTestResponse> {
        return connection.runAndCaptureResponse("Overlay.getHighlightObjectForTest", input, GetHighlightObjectForTestResponse::class.java).map {
            it.value()
        }
    }
    ...
    fun hideHighlight(): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.hideHighlight", null, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun highlightFrame(input: HighlightFrameRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.highlightFrame", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
...
objectId must be specified.
     ...
     */
    fun highlightNode(input: HighlightNodeRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.highlightNode", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun highlightQuad(input: HighlightQuadRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.highlightQuad", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun highlightRect(input: HighlightRectRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.highlightRect", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
...
Backend then generates 'inspectNodeRequested' event upon element selection.
     ...
     */
    fun setInspectMode(input: SetInspectModeRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setInspectMode", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowAdHighlights(input: SetShowAdHighlightsRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowAdHighlights", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setPausedInDebuggerMessage(input: SetPausedInDebuggerMessageRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setPausedInDebuggerMessage", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowDebugBorders(input: SetShowDebugBordersRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowDebugBorders", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowFPSCounter(input: SetShowFPSCounterRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowFPSCounter", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowPaintRects(input: SetShowPaintRectsRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowPaintRects", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowLayoutShiftRegions(input: SetShowLayoutShiftRegionsRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowLayoutShiftRegions", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowScrollBottleneckRects(input: SetShowScrollBottleneckRectsRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowScrollBottleneckRects", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowHitTestBorders(input: SetShowHitTestBordersRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowHitTestBorders", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
     ...
     */
    fun setShowViewportSizeOnResize(input: SetShowViewportSizeOnResizeRequest): io.reactivex.Single<pl.wendigo.chrome.protocol.ResponseFrame> {
        return connection.runAndCaptureResponse("Overlay.setShowViewportSizeOnResize", input, pl.wendigo.chrome.protocol.ResponseFrame::class.java).map {
            it.value()
        }
    }
...
user manually inspects an element.
    ...
    fun inspectNodeRequestedTimed(): io.reactivex.Flowable<io.reactivex.schedulers.Timed<InspectNodeRequestedEvent>> {
        return connection.captureEvents("Overlay.inspectNodeRequested", InspectNodeRequestedEvent::class.java)
    }
    ...
    fun nodeHighlightRequestedTimed(): io.reactivex.Flowable<io.reactivex.schedulers.Timed<NodeHighlightRequestedEvent>> {
        return connection.captureEvents("Overlay.nodeHighlightRequested", NodeHighlightRequestedEvent::class.java)
    }
    ...
    fun screenshotRequestedTimed(): io.reactivex.Flowable<io.reactivex.schedulers.Timed<ScreenshotRequestedEvent>> {
        return connection.captureEvents("Overlay.screenshotRequested", ScreenshotRequestedEvent::class.java)
    }

    /**
     *  Fired when user cancels the inspect mode.
     */
    fun inspectModeCanceled(): io.reactivex.Flowable<pl.wendigo.chrome.protocol.Event> {
     ...
     * Fired when user cancels the inspect mode.
     */
    fun inspectModeCanceledTimed(): io.reactivex.Flowable<io.reactivex.schedulers.Timed<pl.wendigo.chrome.protocol.Event>> {
        return connection.captureEvents("Overlay.inspectModeCanceled", pl.wendigo.chrome.protocol.Event::class.java)
    }
